...
FIG_WIDTH_PER           = 8       # inches per covariate column
...
ION_CHANNEL_Y_OFFSETS = {
    ...
    # add other channel-specific offsets if needed
}
...
def process_file(mat_file_path):
    ...
    def to_float(v):
        # v may be numpy scalar, Python float, or 1x1 sparse
        try:
            if issparse(v):
    ...
    def to_str(v):
        # v may be MATLAB char row, numpy array of chars, or Python str
    ...
    # Case 1: struct-array (one element per parameter)
    if isinstance(peb, np.ndarray):
        ...
        # Case 2: single struct with vector fields
    ...
    # ---------- DEBUG HOOK ----------
    sample_T = [p for p in Pnames if 'T(' in p or re.search(r'\bT\s*\(', p)]
    print("Sample T Pnames:", sample_T[:5])
    # --------------------------------

    # Determine segments (usually 1 with thresholded struct-array)
    Np = len(Pnames)
    if Np == 0:
    ...
    for s in range(nseg):
        ...
        for i, pn in enumerate(Pnames):
            ...
            if m_sh:
                ...
                # Do not return/continue here; we may still parse extrinsic/intrinsic if val != 0

            # Skip zeros ONLY for extrinsic/intrinsic edges
            if val == 0:
                continue
...
def plot_connectivity_for_covariate(ax, cov, data):
    ...
    for conns in grouped_ex.values():
        for idx, (src, tgt, lab, ep) in enumerate(conns):
            ...
            # trim both ends by REGION_RADIUS + EXTRINSIC_TRIM
    ...
    # fix limits (include ion-channel arrows)
...
if __name__ == "__main__":
    file_paths = [
        r"D:\signal_data\cleaning_git\to_plot\Plot_ctbs_final_BST_n1c.mat",
        r"D:\signal_data\cleaning_git\to_plot\Plot_itbs_final_BST_n1i.mat",
        r"D:\signal_data\cleaning_git\to_plot\Plot_sham_final_BST_n1s.mat",
        r"D:\signal_data\cleaning_git\to_plot\Plot_ctbs_BST_-1s_1c.mat",
        r"D:\signal_data\cleaning_git\to_plot\Plot_itbs_BST_-1s_1i.mat",
        r"D:\signal_data\cleaning_git\to_plot\Plot_cTBSvsiTBS_BST_-1c_1i.mat",
    ]
    ...
    # Replace your loop header with this:
    for f_idx, (fp, segs) in enumerate(zip(file_paths, all_segments), start=1):
        # filename -> prefix like "Plot_ctbs_final_BST_n1c"
        ...
        for seg in segs:
            ...
            # Use the real filename in the title, too
            ...
            # ------- filenames based on the real prefix -------
            outdir = r'D:\signal_data\cleaning_git\to_plot'
            os.makedirs(outdir, exist_ok=True)
            base = f"{prefix}_segment{seg['segment']}"
            ...
            subfolders = ['RBG', 'CMYK', 'Grayscale']  # keep as-is, or change 'RBG' -> 'RGB'
            for sub in subfolders:
                os.makedirs(os.path.join(overtime_root, sub), exist_ok=True)

            # 1) SVG copies per subfolder
            for sub in subfolders:
            ...
            # 2) Raster once, then derive CMYK/Gray
            buf = io.BytesIO()
